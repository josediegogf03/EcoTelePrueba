...
def get_theme_aware_css():
    """
    Modern minimal dashboard with coherent design language.
    Features: Fixed layout, smooth transitions, better visual hierarchy.
    """
    return """
<style>
/*
  Modern Minimal Dashboard Design System
  - Fixed layout with smooth transitions
  - Coherent visual hierarchy
  - Enhanced data visualization aesthetics
  - Preserved functionality with improved UX
*/
...
:root {
  ...
  --brand-primary: 220 25% 45%;
  --brand-secondary: 280 20% 55%;
  --brand-accent: 160 25% 45%;
  ...
  /* Neutral Scale */
  --bg-primary: Canvas;
  --bg-secondary: color-mix(in oklab, Canvas 95%, CanvasText);
  --bg-tertiary: color-mix(in oklab, Canvas 90%, CanvasText);
  ...
  /* Glass System */
  --glass-primary: color-mix(in oklab, Canvas 75%, transparent);
  --glass-secondary: color-mix(in oklab, Canvas 65%, transparent);
  --glass-border: color-mix(in oklab, CanvasText 15%, transparent);
  
  /* Shadows */
  --shadow-sm: 0 2px 8px color-mix(in oklab, CanvasText 8%, transparent);
  --shadow-md: 0 8px 24px color-mix(in oklab, CanvasText 12%, transparent);
  --shadow-lg: 0 16px 48px color-mix(in oklab, CanvasText 16%, transparent);
...
@media (prefers-color-scheme: dark) {
  :root {
    --glass-primary: color-mix(in oklab, Canvas 60%, transparent);
    --glass-secondary: color-mix(in oklab, Canvas 50%, transparent);
...
[data-testid="stAppViewContainer"] {
  background: 
    radial-gradient(1200px 800px at 20% -10%, color-mix(in oklab, hsl(var(--brand-primary)) 6%, transparent), transparent 70%),
    radial-gradient(1000px 600px at 80% 110%, color-mix(in oklab, hsl(var(--brand-secondary)) 6%, transparent), transparent 70%),
...
[data-testid="stHeader"] {
  background: linear-gradient(90deg,
    color-mix(in oklab, hsl(var(--brand-primary)) 8%, transparent),
    color-mix(in oklab, hsl(var(--brand-secondary)) 8%, transparent)
  ), var(--glass-primary);
...
.main-header {
  ...
  background: linear-gradient(135deg, 
    color-mix(in oklab, hsl(var(--brand-primary)) 70%, var(--text-primary)),
    color-mix(in oklab, hsl(var(--brand-secondary)) 70%, var(--text-primary))
  );
...
.card {
  ...
  border: 1px solid var(--glass-border);
  background: 
    radial-gradient(120% 130% at 85% 15%, color-mix(in oklab, hsl(var(--brand-secondary)) 4%, transparent), transparent 60%),
    radial-gradient(130% 120% at 15% 85%, color-mix(in oklab, hsl(var(--brand-primary)) 4%, transparent), transparent 60%),
    var(--glass-primary);
...
div[data-testid="stMetric"] {
  ...
  padding: var(--space-md);
  background:
    radial-gradient(120% 140% at 10% 0%, color-mix(in oklab, hsl(var(--brand-primary)) 6%, transparent), transparent 60%),
    radial-gradient(140% 120% at 90% 100%, color-mix(in oklab, hsl(var(--brand-secondary)) 6%, transparent), transparent 60%),
    var(--glass-primary);
  backdrop-filter: blur(16px) saturate(140%);
  border: 1px solid var(--glass-border);
  box-shadow: var(--shadow-md);
...
.gauge-container {
  ...
  border: 1px solid var(--glass-border);
  background:
    radial-gradient(120% 120% at 85% 15%, color-mix(in oklab, hsl(var(--brand-primary)) 3%, transparent), transparent 60%),
    radial-gradient(120% 130% at 20% 80%, color-mix(in oklab, hsl(var(--brand-secondary)) 3%, transparent), transparent 60%),
    var(--glass-primary);
  backdrop-filter: blur(12px);
...
.chart-container {
  ...
  border: 1px solid var(--glass-border);
  background:
    radial-gradient(110% 120% at 85% 10%, color-mix(in oklab, hsl(var(--brand-primary)) 3%, transparent), transparent 60%),
    var(--glass-primary);
  padding: var(--space-sm);
  box-shadow: var(--shadow-md);
  margin: var(--space-md) 0;
...
.stTabs [data-baseweb="tab"][aria-selected="true"] {
  background: linear-gradient(135deg,
    color-mix(in oklab, hsl(var(--brand-primary)) 15%, transparent),
    color-mix(in oklab, hsl(var(--brand-secondary)) 15%, transparent)
  );
  color: var(--text-primary);
  box-shadow: var(--shadow-sm);
}
...
.stButton > button, div[data-testid="stDownloadButton"] > button {
  ...
  background: linear-gradient(135deg,
    color-mix(in oklab, hsl(var(--brand-primary)) 20%, var(--bg-primary)),
    color-mix(in oklab, hsl(var(--brand-secondary)) 20%, var(--bg-primary))
  ) !important;
  border: 1px solid color-mix(in oklab, hsl(var(--brand-primary)) 15%, var(--glass-border)) !important;
...
[data-testid="stDataFrame"], [data-testid="stExpander"], [data-testid="stAlert"] {
  ...
  border: 1px solid var(--glass-border);
  background:
    radial-gradient(120% 120% at 80% 10%, color-mix(in oklab, hsl(var(--brand-primary)) 2%, transparent), transparent 60%),
    var(--glass-primary);
  backdrop-filter: blur(12px);
}
...
::-webkit-scrollbar-thumb:hover {
  background: color-mix(in oklab, hsl(var(--brand-primary)) 30%, var(--glass-border));
}

/* Focus States */
*:focus-visible {
  outline: 2px solid color-mix(in oklab, hsl(var(--brand-primary)) 50%, var(--text-primary));
  outline-offset: 2px;
  border-radius: var(--radius-sm);
}
...
# Apply the modern design system
...
def render_live_gauges(kpis: Dict[str, float], unique_ns: str = "gauges"):
    ...
    st.markdown('<div class="section-header">📊 Live Performance Gauges</div>', unsafe_allow_html=True)

    # Create the gauge grid with new design system
    ...
    st.markdown('</div>', unsafe_allow_html=True)  # Close gauge-grid
...
def main():
    """Main dashboard function with modern minimal design."""
    ...
    with st.sidebar:
        st.markdown('<div class="section-header">🔧 Connection & Data Source</div>', unsafe_allow_html=True)
    ...
    if (
        ...
        st.markdown(
            '<div class="notification">📚 Viewing Historical Data - No auto-refresh active</div>',
            unsafe_allow_html=True,
        )
    ...
    # Status row for populated data with modern styling
    ...
    # Modern tab system with improved styling
    st.markdown('<div class="section-header">📈 Dashboard Analytics</div>', unsafe_allow_html=True)
    ...
    with tabs[8]:
        ...
        st.markdown('<div class="section-header">📃 Raw Telemetry Data</div>', unsafe_allow_html=True)
    ...
    st.markdown(
        "<p>Shell Eco-marathon Telemetry Dashboard with Modern Minimal Design</p>",
        unsafe_allow_html=True,
    )
    st.markdown('</div>', unsafe_allow_html=True)
