...
st.set_page_config(
    page_title="🏎️ Shell Eco-marathon Telemetry",
    page_icon="🏎️",
    layout="wide",
...
# -------------------------------------------------------
# Modern, theme-aware CSS refresh
# -------------------------------------------------------
...
:root {
  ...
  --surface-2: color-mix(in oklab, Canvas 85%, CanvasText);
  --surface-3: color-mix(in oklab, Canvas 80%, CanvasText);
  --surface-glass: color-mix(in oklab, Canvas 80%, transparent);

  --border: color-mix(in oklab, CanvasText 14%, Canvas);
  --border-strong: color-mix(in oklab, CanvasText 22%, Canvas);

  --shadow-sm: 0 4px 18px color-mix(in oklab, CanvasText 10%, transparent);
  --shadow-md: 0 12px 32px color-mix(in oklab, CanvasText 13%, transparent);
  --shadow-lg: 0 18px 50px color-mix(in oklab, CanvasText 16%, transparent);
}
...
@media (prefers-color-scheme: dark) {
  :root {
    ...
    --surface-glass: color-mix(in oklab, Canvas 70%, transparent);
...
def calculate_roll_and_pitch(df: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        ...
        df_calc[['roll_rad', 'roll_deg', 'pitch_rad', 'pitch_deg']] = (
            df_calc[['roll_rad', 'roll_deg', 'pitch_rad', 'pitch_deg']].replace([np.inf, -np.inf, np.nan], 0)
        )
...
def calculate_kpis(df: pd.DataFrame) -> Dict[str, float]:
    ...
    try:
        ...
        numeric_cols = [
            "energy_j", "speed_ms", "distance_m", "power_w", "voltage_v", "current_a",
            "latitude", "longitude", "altitude", "roll_deg", "pitch_deg"
        ]
...
def create_small_gauge(
    ...
    fig = go.Figure(
        go.Indicator(
            mode=mode,
            value=value,
            number={"suffix": suffix, "font": {"size": 18, "color": "#fff"}},
            delta=delta_cfg,
            gauge=gauge,
            domain={"x": [0, 1], "y": [0, 1]},
        )
...
def render_live_gauges(kpis: Dict[str, float], unique_ns: str = "gauges"):
    ...
    with cols[1]:
        ...
        battery_fig = create_small_gauge(kpis["battery_percentage"], 100, "Battery", "#2ca02c", "%")
    ...
    with cols[4]:
        ...
        roll_fig = create_small_gauge(kpis["current_roll_deg"], roll_max, "Roll", "#e377c2", "°")
    ...
    with cols[5]:
        ...
        pitch_fig = create_small_gauge(kpis["current_pitch_deg"], pitch_max, "Pitch", "#17becf", "°")
...
def analyze_data_quality(df: pd.DataFrame, is_realtime: bool):
        ...
        try:
            ...
            if time_since_last > threshold:
                notifications.append(
                    ...
                    f"(Expected update ~{avg_rate:.1f}s)"
                )
        except Exception as e:
    ...
    if all_sensors_failing and len(failing_sensors) > 3:
        ...
        notifications.append(
            f"⚠️ **Sensor Anomaly:** The following sensor(s) may be unreliable: **{sensor_list}**."
        )

    st.session_state.data_quality_notifications = notifications


def base_plotly_layout(fig: go.Figure, height: int = 400) -> go.Figure:
    fig.update_layout(
        height=height,
        plot_bgcolor="rgba(0,0,0,0)",
        paper_bgcolor="rgba(0,0,0,0)",
        font={"color": "var(--text)", "family": "Inter, sans-serif"},
        title_font={"color": "var(--text)", "family": "Inter, sans-serif"},
        template="none",
...
def create_power_chart(df: pd.DataFrame):
    ...
    fig.update_layout(title_text="⚡ Electrical System Performance")
    return base_plotly_layout(fig, height=520)
...
def create_imu_chart(df: pd.DataFrame):
    ...
    fig = make_subplots(
        rows=3, cols=1,
        subplot_titles=("🎯 Gyroscope Data (deg/s)", "📈 Accelerometer Data (m/s²)", "🎭 Roll & Pitch (degrees)"),
        vertical_spacing=0.15,
    )
    ...
    fig.add_trace(
        go.Scatter(x=df["timestamp"], y=df["roll_deg"], name="Roll (°)", line=dict(color="#e377c2", width=3)),
        row=3, col=1,
    )
    fig.add_trace(
        go.Scatter(x=df["timestamp"], y=df["pitch_deg"], name="Pitch (°)", line=dict(color="#17becf", width=3)),
        row=3, col=1,
    )

    fig.update_layout(title_text="⚡ IMU System Performance with Roll & Pitch", legend=dict(bgcolor="rgba(0,0,0,0)"))
    return base_plotly_layout(fig, height=720)
...
def create_imu_detail_chart(df: pd.DataFrame):
    ...
    fig = make_subplots(
        rows=3, cols=3,
        subplot_titles=("🌀 Gyro X", "🌀 Gyro Y", "🌀 Gyro Z", "📊 Accel X", "📊 Accel Y", "📊 Accel Z", "🔄 Roll (°)", "📐 Pitch (°)", "🎯 R&P Combined"),
    ...
    fig.add_trace(
        go.Scatter(x=df["timestamp"], y=df["roll_deg"], name="Roll", line=dict(color="#e377c2", width=3), showlegend=False),
        row=3, col=1,
    )
    fig.add_trace(
        go.Scatter(x=df["timestamp"], y=df["pitch_deg"], name="Pitch", line=dict(color="#17becf", width=3), showlegend=False),
        row=3, col=2,
    )
    fig.add_trace(
        go.Scatter(x=df["timestamp"], y=df["roll_deg"], name="Roll", line=dict(color="#e377c2", width=2), showlegend=False),
        row=3, col=3,
    )
    fig.add_trace(
        go.Scatter(x=df["timestamp"], y=df["pitch_deg"], name="Pitch", line=dict(color="#17becf", width=2), showlegend=False),
        row=3, col=3,
    )

    fig.update_layout(title_text="🎮 Detailed IMU Sensor Analysis with Roll & Pitch")
    return base_plotly_layout(fig, height=720)
...
def create_gps_map_with_altitude(df: pd.DataFrame):
    """Modernized: use mapbox subplot for the track + altitude profile."""
    ...
    color_vals = df_valid["speed_ms"] if "speed_ms" in df_valid.columns else None
    fig.add_trace(
        go.Scattermapbox(
            ...
            marker=dict(
                size=8,
                color=color_vals if color_vals is not None else "#1f77b4",
                colorscale="Plasma",
    ...
    if "altitude" in df.columns and not df["altitude"].dropna().empty:
        altitude_data = df.dropna(subset=["altitude"]).copy()
        altitude_data = altitude_data[altitude_data["altitude"] != 0]
        if altitude_data.empty:
            fig.add_trace(
                go.Scatter(x=[0], y=[0], mode="text", text=["No valid altitude data"], showlegend=False),
                row=1, col=2,
            )
        else:
            fig.add_trace(
        ...
        fig.add_trace(
            go.Scatter(x=[0], y=[0], mode="text", text=["No altitude data"], showlegend=False),
            row=1, col=2,
        )
    ...
    return base_plotly_layout(fig, height=540)
...
def render_dynamic_charts_section(df: pd.DataFrame):
    if not st.session_state.chart_info_initialized:
        ...
        <div class="chart-type-grid" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(210px,1fr));gap:.75rem;">
            <div class="card"><div class="chart-type-name" style="font-weight:800;color:var(--accent)">📈 Line Chart</div><div class="chart-type-desc" style="color:var(--text-secondary)">Time series & trends</div></div>
            <div class="card"><div class="chart-type-name" style="font-weight:800;color:var(--accent)">🔵 Scatter Plot</div><div class="chart-type-desc" style="color:var(--text-secondary)">Correlations</div></div>
            <div class="card"><div class="chart-type-name" style="font-weight:800;color:var(--accent)">📊 Bar Chart</div><div class="chart-type-desc" style="color:var(--text-secondary)">Compare recent values</div></div>
            <div class="card"><div class="chart-type-name" style="font-weight:800;color:var(--accent)">📉 Histogram</div><div class="chart-type-desc" style="color:var(--text-secondary)">Distributions</div></div>
            <div class="card"><div class="chart-type-name" style="font-weight:800;color:var(--accent)">🔥 Heatmap</div><div class="chart-type-desc" style="color:var(--text-secondary)">Correlation matrix</div></div>
        </div>
        """
...
def main():
        ...
        if st.session_state.data_source_mode == "realtime_session":
            ...
            if st.session_state.telemetry_manager:
                ...
                if st.session_state.telemetry_manager.is_connected:
                    st.markdown('<div class="status-indicator"><span class="status-dot"></span> Real-time Connected</div>', unsafe_allow_html=True)
                else:
                ...
                with col2:
                    ...
                    if stats["last_message_time"]:
                        time_since = (datetime.now(timezone.utc) - stats["last_message_time"]).total_seconds()
    ...
    if st.session_state.is_viewing_historical and st.session_state.selected_session:
        st.markdown('<div class="historical-notice">📚 Viewing Historical Data - No auto-refresh active</div>', unsafe_allow_html=True)
        ...
        with col1:
            if st.session_state.data_source_mode == "realtime_session":
                st.info(
                    ...
                    "3. Large sessions are automatically paginated"
                )
            else:
                st.info(
                    "**Getting Started (Historical):**\n"
                    "1. Click 'Refresh Sessions' in the sidebar\n"
                    "2. Select a session to load data\n"
                    "3. Large datasets use pagination"
                )
        with col2:
            with st.expander("🔍 Debug Information"):
                debug_info = {
                    ...
                    if st.session_state.selected_session else None,
    ...
    with st.container():
        ...
        with col3:
            last_update_local = st.session_state.last_update.astimezone()
            st.info(f"⏰ Last update: {last_update_local.strftime('%H:%M:%S')}")
        with col4:
    ...
    if len(df) > 10000:
        ...
        <div class="pagination-info">
            <strong>📊 Large Dataset Loaded:</strong> {len(df):,} data points retrieved via pagination
        </div>
        """,
            unsafe_allow_html=True,
        )
        ...
        with st.expander("📊 Dataset Statistics"):
            ...
            with col2:
                if "timestamp" in df.columns and len(df) > 1:
                    try:
                        timestamp_series = pd.to_datetime(df["timestamp"], errors="coerce", utc=True).dropna()
    ...
    # Footer
    st.divider()
    st.markdown("<div class='app-footer'>Shell Eco-marathon Telemetry Dashboard • Modern UI refresh</div>", unsafe_allow_html=True)
