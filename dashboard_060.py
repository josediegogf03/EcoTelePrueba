...
def calculate_kpis(df: pd.DataFrame) -> Dict[str, float]:
    ...
    try:
        numeric_cols = [
            ...
            "gyro_z",
            "voltage_v",
            "current_a",  # Added for power chart calculations
            "accel_x",
            "accel_y",
            "accel_z",  # Added for IMU chart calculations
            "latitude",
            "longitude",  # Added for GPS map calculations
        ]
        ...
        if all(col in df.columns for col in ["accel_x", "accel_y", "accel_z"]):
            ...
            if not accel_x.empty and not accel_y.empty and not accel_z.empty:
                total_acceleration_calculated = np.sqrt(
                ...
                "total_acceleration" in df.columns
            ):  # Fallback to existing if calculated isn't possible
        ...
        if all(col in df.columns for col in ["gyro_x", "gyro_y", "gyro_z"]):
            ...
            if not gyro_data_df.empty:
                gyro_magnitude = np.sqrt(
                    gyro_data_df["gyro_x"] ** 2
                    + gyro_data_df["gyro_y"] ** 2
                    + gyro_data_df["gyro_z"] ** 2
                )
...
def create_gps_map(df: pd.DataFrame):
    ...
    fig = px.scatter_map(
        ...
        if all(col in df_valid.columns for col in ["speed_ms", "power_w", "voltage_v"])
        else None,
...
def create_dynamic_chart(df: pd.DataFrame, chart_config: Dict[str, Any]):
    ...
    if chart_type == "heatmap":
        ...
        try:
            if chart_type == "line":
                fig = px.line(
                    df, x=x_col, y=y_col, title=title, color_discrete_sequence=["#1f77b4"]
                )
                ...
                fig = px.bar(
                    recent_df, x=x_col, y=y_col, title=title, color_discrete_sequence=["#2ca02c"]
                )
            elif chart_type == "histogram":
                # Histogram uses only one axis (the value itself) for distribution
                fig = px.histogram(
                    df, x=y_col, title=f"Distribution of {y_col}", color_discrete_sequence=["#d62728"]
                )
            ...
            try:
                with st.container(border=True):
                    ...
                    try:
                        ...
                        if fig:
                            st.plotly_chart(
                                fig, use_container_width=True, key=f"chart_plot_{chart_config['id']}"
                            )
...
def main():
        ...
        if st.session_state.telemetry_manager and st.session_state.telemetry_manager.is_connected:
            ...
            if new_messages and "session_id" in new_messages[0]:
                ...
                if (
                    ...
                    or st.session_state.telemetry_data.empty
                ):  # Load full session initially or if session changes
                    st.session_state.current_session_id = current_session_id
    ...
    with tabs[8]:
        ...
        st.warning(
            "ℹ️ Only the **last 100 datapoints** are displayed below. Download the CSV for the complete dataset."
        )
    ...
    if (
        ...
        and st.session_state.telemetry_manager
